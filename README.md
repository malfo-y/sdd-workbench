...
- 원격 워크스페이스에서 iTerm SSH / VSCode Remote-SSH 외부 도구 열기
- 워치 모드 제어(`Auto` / `Native` / `Polling`) + fallback
- 파일 트리 Git 배지(U/M) + 활성 파일 라인 Git 마커
- Ayu Mirage `dark` / `light` 테마 + native `View > Theme` 메뉴
...
- `Open In` 실행(iTerm / VSCode / Finder, 원격 워크스페이스는 SSH/Remote-SSH로 연결)
- 파일 트리 탐색
...
클립보드:
- 내부 Copy/Paste: 앱 내에서 파일/디렉토리 복사
- macOS Finder 클립보드: Finder에서 복사한 파일을 앱에서 붙여넣기 가능
