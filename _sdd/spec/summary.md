...
**생성일** (Generated): 2026-03-18
**스펙 버전** (Spec Version): 0.49.1
**최종 업데이트** (Last Updated): 2026-03-18
...
SDD Workbench는 로컬/원격 워크스페이스에서 코드, Markdown 스펙, 코멘트 export 흐름을 한 작업대에서 다루는 Electron 앱이다. 현재 canonical 경로는 `main.md` whitepaper와 6개 컴포넌트 supporting docs이며, 원격 경로는 F27 Remote Agent Protocol이 단일 기준선이다. comments/export 영역은 이제 `View Comments`, `Add Comment`, `Add Global Comments`, `Export Comments` 전부 draggable modal contract를 공유하고, spec viewer는 Python citation navigation을 prose / inline code / generic fenced block까지 지원한다.
