...
4. polling 모드는 기본 1500ms 간격 메타데이터 diff(`mtimeMs + size`)로 변경 이벤트를 생성한다.
5. watchStart 응답에는 `watchMode`, `isRemoteMounted`, `fallbackApplied`가 포함된다.
6. F15(SSHFS 기반) 연결 경로는 폐기되었고, 원격 연결은 F27(remote-protocol)만 사용한다.
...
### 5.9 Remote Agent Protocol 원격 워크스페이스 플로우(F27, Implemented)

1. `workspace:connectRemote` 요청으로 원격 연결 프로필(workspaceId/host/user/port/remoteRoot/agentPath/identityFile)을 Main에 전달한다.
2. Main은 SSH transport를 통해 원격 agent runtime을 배포(덮어쓰기)하고 protocol handshake/healthcheck를 수행한다.
   (MVP 자동화: 존재 확인/미존재 설치 수준을 넘어서, 현재 빌드 runtime으로 동기화하기 위해 매 연결 시 설치 스크립트를 실행)
3. handshake 성공 시 workspace backend를 `remote`로 선택하고 이후 `workspace:index/read/write/create/delete/rename/watch/git/comments`를 remote backend로 라우팅한다.
4. 원격 watch 이벤트는 기존 `workspace:watchEvent` payload 계약으로 변환되어 Renderer로 전달된다.
5. 연결 끊김/타임아웃 시 `remoteConnectionState`를 `degraded`/`disconnected`로 갱신하고 자동 재시도 후 사용자 수동 재시도 경로를 노출한다.
