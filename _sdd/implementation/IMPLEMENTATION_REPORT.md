...
- Total Tasks: 4
- Completed: 4
- Tests Added/Updated: 3 files added or expanded
- All Passing: Yes
...
| Iteration | AC Status (MET/Total) | Critical | High | Re-executed Tasks | Result |
|-----------|----------------------|----------|------|-------------------|--------|
| 1 | 11 / 12 | 0 | 1 | `T3`, `T4` | partial |
| 2 | 12 / 12 | 0 | 0 | `T3`, `T4` | pass |

Iteration note:
- Iteration 1에서 line-based marker count resolution이 유지되어 paragraph marker count regression이 발생했다.
- Iteration 2에서 anchor-key marker mapping과 baseline count merge를 분리해 table cell placement와 기존 count aggregation을 동시에 만족시켰다.

### Completed Tasks

- [x] Task T1: table cell leaf wrapping contract를 fresh regression으로 고정하고 same-cell metadata 전제 조건을 검증
- [x] Task T2: resolver에 same-cell only guard와 repeated-text uniqueness check를 추가
- [x] Task T3: spec viewer panel marker mapping을 line fan-out에서 anchor-key matching으로 전환
- [x] Task T4: table exact-offset, cross-cell fallback, repeated-text ambiguity, table marker placement regression 추가
...
- `tsconfig.node.tsbuildinfo` was generated by `tsc`/`build`.
...
- Earlier `npm test -- --runInBand` invocation failed because Vitest in this repo does not support `--runInBand`. This was a command misuse, not an implementation regression.

### Quality Assessment

- Selection safety: same-cell table selections now keep exact offsets only when cell-local metadata and uniqueness checks support them; cross-cell selections deliberately degrade to line-only range.
- Ambiguity handling: repeated rendered text inside a candidate source span no longer produces optimistic exact offsets, reducing false-positive `Go to Source` and `Add Comment` anchors.
- Marker placement: table-origin comments now resolve against rendered anchor keys so markers land on the intended cell instead of fanning out to every rendered node sharing the same source line.
- Compatibility: paragraph marker counts and hover ordering remain intact because count aggregation still honors `commentLineCounts` while comment payload ordering stays sorted by `compareCodeComments`.
- Scope control: token-level parsing was intentionally excluded; this iteration stops at same-cell exact mapping plus common ambiguity hardening.

### Issues Found

| # | Severity | Description | Phase | Status |
|---|----------|-------------|-------|--------|
| 1 | High | Initial anchor-key rollout dropped legacy count aggregation semantics and caused a paragraph marker count regression | Iteration 1 / T3-T4 | resolved |

### Conclusion

READY — F48 table cell + repeated-text anchor hardening 구현 완료. same-cell exact offsets, cross-cell safe fallback, repeated-text ambiguity guard, and cell-level marker placement are now covered by fresh regression tests and full verification.

---

## F48-R1 Addendum (2026-04-02)

### Progress Summary

- Trigger: implementation review follow-up
- Completed follow-up tasks: 2
- Tests Added/Updated: 1 file
- All Passing: Yes

### Completed Tasks

- [x] Review finding fix 1: same-line multi-cell table comments가 per-cell marker count를 과대 계산하지 않도록 count aggregation 보정
- [x] Review finding fix 2: offset-less legacy/imported table comments가 arbitrary cell이 아니라 neutral table anchor에 붙도록 fallback 조정
...
- Count correctness: exact-anchor comment entries가 이미 로드된 line에서는 그 entry count를 source of truth로 삼고, baseline line count의 remainder만 fallback anchor에 투영해 same-row multi-cell overcount를 제거했다.
- Neutral fallback: offset-less table comments는 cell precision을 가장하지 않고 neutral table anchor로 표시되도록 조정했다.
- DOM validity: neutral table marker는 sibling placement로 렌더해 invalid `<span> -> <table>` nesting warning까지 제거했다.

### Conclusion

READY — implementation review에서 지적된 medium findings 2건을 반영했다. same-line multi-cell count aggregation과 offset-less table fallback 모두 fresh regression으로 고정했고, full test/build까지 다시 통과했다.
