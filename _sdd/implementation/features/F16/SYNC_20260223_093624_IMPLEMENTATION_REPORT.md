...
- Plan source: `/_sdd/drafts/feature_draft_f12_2_view_comments_edit_delete.md` (Part 2)
- Completed:
  - `View Comments` 헤더 액션과 코멘트 목록 모달 오케스트레이션 추가
  - 코멘트 목록 전용 UI(`comment-list-modal`) 도입: 정렬 목록, 메타데이터 표기, 긴 본문 접기/펼치기
  - 코멘트 본문 편집/저장 플로우 추가(`sanitizeCommentBody` 재사용, immutable update)
  - 코멘트 개별 삭제 + `Delete Exported` 일괄 삭제(2-step confirm) 추가
  - 저장/삭제 결과가 `comments.json` source-of-truth를 통해 배지/카운트에 즉시 반영되도록 연동
  - 단위/통합 테스트 보강 및 전체 품질 게이트 통과
  - Verification: `npm test`, `npm run lint`, `npm run build` pass
...
- F11/F11.1 코멘트 저장/마커 파이프라인 위에 관리 UI만 증설해 기존 export/add 흐름 회귀를 방지했다.
- 편집/삭제는 공통 `saveComments` 경로를 재사용해 에러 배너/쓰기 락 정책이 일관된다.
- `Delete Exported`는 `exportedAt` 존재 여부 기준으로만 동작해 pending 코멘트가 보존된다.
- F12.1 hover popover와 동시 적용 시에도 라인 배지 count가 동일 source 데이터를 공유해 일관된다.

### 4) Issue Table (Severity / Status)

| Severity | Issue | Status |
|----------|-------|--------|
| Improvement | 코멘트 검색/필터/정렬 사용자 옵션은 미지원 | backlog |
| Improvement | 대량 코멘트에서 virtualization/list paging 최적화는 미적용 | backlog |

### 5) Final Conclusion (F12.2)
...
- Plan source: `/_sdd/drafts/feature_draft_f12_5_comment_feedback_autodismiss_and_action_group_clarity.md` (Part 2)
- Completed:
  - 코멘트 배너 전용 helper(`showCommentBanner`) + 5초 auto-dismiss 타이머 도입
  - `View Comments` 모달에 global comments 상단 read-only 섹션 및 empty 상태 표시 추가
  - Export 모달에 global comments 포함 여부(`included`/`not included`) 명시
  - 헤더 액션을 `Code comments` 그룹과 `Workspace` 그룹으로 재구성
  - workspace 그룹 내부 순서를 `WorkspaceSwitcher -> Open -> Close`로 정리
  - App/CommentListModal 테스트 보강(배너 자동 dismiss, 그룹 순서, global 렌더/상태 표시)
  - Verification: `npm test`, `npm run lint`, `npm run build` pass
...
- 코멘트 배너 자동 닫힘은 코멘트 경로로 한정되어 workspace/indexing 에러 배너 정책과 충돌하지 않는다.
- F12.2/F12.3의 데이터 계약(`comments.json`, `global-comments.md`)은 변경 없이 UI 계층만 확장했다.
- 헤더 그룹 재배치 후에도 기존 버튼의 접근성 이름(`aria-label`)과 disabled 로직은 유지된다.
- Export 정책 자체는 유지하면서 사전 가시성만 높여 사용자 혼동을 줄였다.

### 4) Issue Table (Severity / Status)

| Severity | Issue | Status |
|----------|-------|--------|
| Improvement | 코멘트 배너 분류 체계는 문자열 기반 helper에 의존(타입 기반 분류 미도입) | backlog |
| Improvement | `View Comments`의 global 섹션은 현재 read-only이며 인라인 편집은 미지원 | backlog |

### 5) Final Conclusion (F12.5)

- `READY`
- Reason: F12.5 계획 태스크(T1~T5)를 완료했고 자동 테스트/린트/빌드 게이트를 통과함.
