...
- Active plan: `/_sdd/drafts/feature_draft_f38_native_theme_menu_and_header_compaction.md` (Part 2)
- Covered tasks:
  - Phase 1: `1` (completed)
  - Phase 2: `2, 3` (completed)
  - Phase 3: `4, 5` (completed)

| ID | Task | Priority | Dependencies | Status | Tests |
|----|------|----------|--------------|--------|-------|
| 1 | main/preload/renderer appearance menu bridge 정의 | P0 | - | completed | `appearance-theme.test.ts`, `App.test.tsx`, `electron/appearance-menu.test.ts` pass |
| 2 | role-preserving application menu template + `View > Theme` submenu 추가 | P0 | 1 | completed | `electron/appearance-menu.test.ts` pass |
| 3 | renderer bootstrap/theme change를 menu checked state와 동기화 | P0 | 1, 2 | completed | `appearance-theme.test.ts`, `App.test.tsx` pass |
| 4 | header large theme group 제거 및 레이아웃 정리 | P1 | 2, 3 | completed | `App.test.tsx` pass |
| 5 | native menu sync + header compaction 회귀 테스트 추가 | P0 | 2, 3, 4 | completed | `appearance-theme.test.ts`, `App.test.tsx`, `electron/appearance-menu.test.ts`, `npm test`, `npm run build` pass |

### 2) Files Changed (F38)

- `electron/appearance-menu.ts` (new)
- `electron/appearance-menu.test.ts` (new)
- `electron/main.ts`
- `electron/preload.ts`
- `electron/electron-env.d.ts`
...
### 3) Test and Quality Gate Status (F38)
...
### 4) Parallel Groups Executed (F38)

- Group A: `1`, `2`, `3`
  - Parallel target files가 일부 겹치고, main/preload/renderer IPC contract가 같은 기능 축에 묶여 있어서 single-session TDD로 진행했다.
- Group B: `4`, `5`
  - Header compaction과 회귀 테스트가 `App.tsx`/`App.test.tsx`에 함께 수렴하므로 sequential execution이 안전했다.

### 5) Blockers and Decisions (F38)

- Blockers: 없음
- Applied decisions:
  - renderer `appearanceTheme` + localStorage를 authoritative source로 유지하고, Electron main menu는 IPC sync로 checked state를 반영한다.
  - native menu는 새 top-level `Theme` 메뉴가 아니라 `View > Theme` radio submenu로 추가했다.
  - `setApplicationMenu()` 도입 시 role-preserving template를 사용해 `file/edit/view/window/help` 표준 메뉴 동작을 유지했다.
  - header의 large `Theme` group은 제거하고, 중복 control 없이 native menu를 primary entry point로 승격했다.
  - boot 시점 checked state 지연을 줄이기 위해 `src/main.tsx`의 pre-paint restore 직후 host sync를 한 번 수행하고, `App.tsx`에서도 state change마다 sync를 유지했다.
...
- Active plan: `/_sdd/drafts/feature_draft_f41_f42_draggable_comment_modals.md` (Part 2)
- Covered tasks:
  - Phase 1: `T1, T2` (completed)
  - Phase 2: `T3, T4` (completed)
  - Phase 3: `T5, T6` (completed)
...
| ID | Task | Priority | Dependencies | Status | Tests |
|----|------|----------|--------------|--------|-------|
| T1 | shared modal drag hook / clamp utility 추가 | P0 | - | completed | `modal-drag-position.test.ts`, `tsc --noEmit` pass |
| T2 | View Comments에 draggable header 적용 | P0 | T1 | completed | `comment-list-modal.test.tsx`, `App.test.tsx` pass |
| T3 | Add Comment / Add Global Comments draggable 적용 | P1 | T1 | completed | `comment-editor-modal.test.tsx`, `global-comments-modal.test.tsx` pass |
| T4 | Export Comments draggable 적용 | P1 | T1 | completed | `export-comments-modal.test.tsx`, `App.test.tsx` pass |
| T5 | drag utility + modal unit tests 추가 | P0 | T2, T3, T4 | completed | targeted `vitest` suite pass |
| T6 | app-level integration regression 보강 | P0 | T2, T3, T4 | completed | `App.test.tsx`, full `npm test` pass |

### 2) Files Changed (F41/F42)

- `src/modal-drag-position.ts` (new)
- `src/modal-drag-position.test.ts` (new)
- `src/code-comments/comment-list-modal.tsx`
- `src/code-comments/comment-list-modal.test.tsx`
...
- `_sdd/implementation/IMPLEMENTATION_PROGRESS.md`
- `_sdd/implementation/IMPLEMENTATION_REPORT.md`
- `_sdd/implementation/IMPLEMENTATION_INDEX.md`

### 3) Test and Quality Gate Status (F41/F42)
...
### 4) Parallel Groups Executed (F41/F42)

- Group A (sequential fallback): `T1 -> T2`
  - Reason: shared drag contract가 `comment-list-modal.tsx`와 `App.css`에 바로 소비되고 같은 modal shell 구조를 함께 다듬어야 해서 single-session 구현이 안전했다.
- Group B (sequential fallback): `T3 -> T4`
  - Reason: form modal family가 같은 header/style contract를 공유하고 `App.css` 충돌이 확실해 병렬 분리 이점이 낮았다.
- Group C (sequential fallback): `T5 -> T6`
  - Reason: `App.test.tsx`와 modal-level tests가 같은 회귀 경로를 검증하므로 utility/unit 검증을 먼저 고정한 뒤 integration으로 확장했다.

### 5) Blockers and Decisions (F41/F42)

- Blockers: 없음
- Applied decisions:
  - draggable interaction은 comment modal family로만 제한하고, reopen persistence/localStorage restore는 이번 범위에서 제외했다.
  - drag는 modal body가 아니라 dedicated header handle에서만 시작되도록 고정해 textarea, checkbox, button, internal scroll과 충돌하지 않게 했다.
