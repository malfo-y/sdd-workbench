...
describe('workspace-backend/local-workspace-backend', () => {
  it('delegates calls to provided handlers', async () => {
    const handlers = {
      index: vi.fn(async () => ({ ok: true, fileTree: [], source: 'index' })),
      indexDirectory: vi.fn(async () => ({
        ok: true,
        children: [],
      ...
      readFile: vi.fn(async () => ({ ok: true, content: null, source: 'readFile' })),
      ...
      getGitFileStatuses: vi.fn(async () => ({
        ok: true,
        statuses: {},
        source: 'fileStatuses',
      })),
      readComments: vi.fn(async () => ({ ok: true, comments: [], source: 'readComments' })),
      writeComments: vi.fn(async () => ({ ok: true, source: 'writeComments' })),
      readGlobalComments: vi.fn(async () => ({
        ok: true,
        body: '',
        source: 'readGlobalComments',
      })),
